...
class VLSLSTM(nn.Module):
    ...
    def forward(self, x, lengths_in, lengths_aureg, mask_aureg):
        ...
        hi = torch.zeros(self.num_layers * self.num_directions, this_batch_size, self.hidden_size).to(x.device)
        ci = torch.zeros(self.num_layers * self.num_directions, this_batch_size, self.hidden_size).to(x.device)
        ...
        if self.num_directions == 2:
            outputs_teafo_pad = outputs_teafo_pad[:, :, :self.hidden_size]
    ...
    def auto_regressive_loop(self, input_aureg_init, hi, ci, lengths_aureg, mask_aureg):
        ...
        for t in range(max_len):
            ...
            temp_output[current_mask] = output[:, :, :self.hidden_size]  # Ghi output của các chuỗi thực # chỉ những chuỗi có mask = 1 mới được gán giá trị, còn lại vẫn giữ nguyên giá trị 0
...
class NAEDynamicLSTM():
    def __init__(self, input_size, hidden_size, output_size, num_layers_lstm, lr, 
                 num_epochs, batch_size_train, batch_size_val, save_interval, thrown_object, train_id, dropout_rate, warmup_steps=0, loss2_weight=1.0, loss2_1_weight=0.0, weight_decay=1e-4, lstm_bidirectional=False,
        ...
        self.vls_lstm = VLSLSTM(hidden_size, hidden_size, num_layers_lstm, dropout_rate=dropout_rate, bidirectional=lstm_bidirectional).to(device)

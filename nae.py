...
def main():
    ...
    num_samples = 200
    nae = NAE(**model_params, device=DEVICE)  
    data_train = nae.generate_parabolic_motion_data(**one_parabol_params, num_samples=num_samples)
    training_params = {
        'seq_length': 30,
        'prediction_steps': 20,
        'num_epochs': 10000,
    ...
    # # ===================== TRAINING =====================
    # nae.init_wandb(project_name='nae-debug', 
    #             lr=model_params['lr'], 
    #             num_epochs=training_params['num_epochs'], 
    #             batch_size=training_params['batch_size'], 
    #             seq_length=training_params['seq_length'], 
    #             prediction_steps=training_params['prediction_steps'],
    #             data_dir="parabola_generation_function")
    # print('TRAINING NAE MODEL')
    # saved_model_dir = nae.train(data_train, **training_params)
    saved_model_dir = "/home/huynn/huynn_ws/edge-server-project/isaac_sim_learning/isaacsim_nae/NAE/models/19-06-2024_18-57-08/epochs10000_data200_batchsize100_seq30_pred20_timemin13-84"
    # ===================== PREDICT =====================
    print('PREDICTING')
    num_trials = 5
    testing_data = nae.generate_parabolic_motion_data(**one_parabol_params, num_samples=num_trials)
    testing_data = torch.tensor(testing_data, dtype=torch.float32)
    test_prediction_data, test_labels_data = nae.predict_multi_times(num_trials=num_trials, 
                                            testing_data=testing_data, 
                                            seq_length=training_params['seq_length'],
                                            prediction_steps=training_params['prediction_steps'], 
                                            saved_model_dir=saved_model_dir)

    nae.utils.plotter.plot_prediction(test_prediction_data, test_labels_data, swap_y_z=False)

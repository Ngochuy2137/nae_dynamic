...
def main():
    ...
    data_train = input_label_generator.generate_input_label_dynamic_seqs(data_train_raw, step_start=5, step_end=-3, increment=1, shuffle=True)
    data_val = input_label_generator.generate_input_label_dynamic_seqs(data_val_raw, step_start=5, step_end=-3, increment=1, shuffle=True)
    ...
    # mean_ade_entire, mean_ade_future, \
    # mean_nade_entire, mean_nade_future, \
    # mean_final_step_err, capture_success_rate = nae.validate_and_score(data=data_val, batch_size=1024, shuffle=False, inference=True)

    # input_data = data_test[0:10]
    # input_data = [data_test[0], data_test[100]]
    ...
    predicted_seqs, label_seqs = nae.validate_and_score(data=input_data, batch_size=1024, shuffle=False, inference=True)

    print('predicted_seqs, label_seqs: ', len(predicted_seqs), ' ', len(label_seqs))
    input()
    util_plotter = Plotter()

    # input('predicted_seqs shape: ' + str(len(predicted_seqs)))
    # for pred, la in zip(predicted_seqs, label_seqs):
    #     print('check predict, label shape: ', pred.shape , ' ', la.shape)
    #     # check if they are equal
        
    #     # print('     ', np.equal(pred, la))
    #     # print('check if they are equal: ', np.allclose(pred, la))

    # input()
    # for _ in range(10):
    #     random.seed(seed)
    #     random.shuffle(predicted_seqs)
    #     random.seed(seed)
    #     random.shuffle(label_seqs)

    # # get random indices
    # random_indices = random.sample(range(len(predicted_seqs)), 5)
    # predicted_seqs = [predicted_seqs[i] for i in random_indices]
    # label_seqs = [label_seqs[i] for i in random_indices]

    # util_plotter.plot_trajectory_dataset(predicted_seqs, title=thrown_object + '_label', rotate_data_whose_y_up=True, save_plot=False)

    input_data_plot = [inp[0] for inp in input_data]
    # convert all elements of input_data_plot to numpy
    input_data_plot = [inp.cpu().numpy() for inp in input_data_plot]

    # util_plotter.plot_predictions(input_data_plot[0:1], label_seqs[0:1], lim_plot_num=None, rotate_data_whose_y_up=False, title='0')
    # util_plotter.plot_predictions(input_data_plot[1:2], label_seqs[1:2], lim_plot_num=None, rotate_data_whose_y_up=False, title='1')
    # util_plotter.plot_predictions(input_data_plot[2:3], label_seqs[2:3], lim_plot_num=None, rotate_data_whose_y_up=False, title='2')
    
    for in_check, la_check in zip(input_data_plot, label_seqs):
        ...
        print('check if they are equal: ', same)
        if not same:
            util_plotter.plot_predictions([in_check], [la_check], lim_plot_num=None, rotate_data_whose_y_up=False, title=f'{same}')
    print('oke')


    input()
    util_plotter.plot_predictions_plotly(inputs=input_data_plot, labels=label_seqs, predictions=predicted_seqs, title='plane', rotate_data_whose_y_up=True, save_plot=False)

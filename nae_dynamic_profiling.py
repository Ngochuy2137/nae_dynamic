...
class VLSLSTM(nn.Module):
    ...
    def forward(self, x, lengths_in, lengths_aureg, mask_aureg):
        ...
        #----- teacher forcing -----
        with record_function(f"### TEACHER FORCING"):
    ...
    def auto_regressive_loop(self, input_aureg_init, hi, ci, lengths_aureg, mask_aureg):
        ...
        for t in range(max_len):
            with record_function(f"### STEP LSTM {t}"):
                ...
                # check device of current_mask 
                with record_function("@@@ 1 - LSTM"):
            ...
            for epoch in range(start_epoch, self.num_epochs):
                ...
                for batch_idx, batch in enumerate(dataloader_train):
                    ...
                    if batch_idx == 10:
                        ...
                        # get dir of folder including this script
                        cur_dir = os.path.dirname(os.path.realpath(__file__))
                        profiler_dir = os.path.join(cur_dir, 'log_profiling', 'log_profiling')
                        self.util_printer.print_green(f'Profiler dir: {profiler_dir}')
                        with profile(
                            activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                            on_trace_ready=torch.profiler.tensorboard_trace_handler(profiler_dir),
                            record_shapes=True,
                            profile_memory=True,
                            with_stack=True       # Ghi lại stack trace cho các hoạt động
                        ) as prof:
                            with record_function("model_training_step"):
                                with record_function("### LOAD DATA"):
                                    ...
                                    # print('Check contiguous: ', inputs_pad.is_contiguous(), ' ', labels_teafo_pad.is_contiguous(), ' ', labels_aureg_pad.is_contiguous(), ' ', labels_reconstruction_pad.is_contiguous())
                                ...
                                with autocast(device_type='cuda'):
                                    ...
                                    with record_function("### LOSS CALCULATION"):
                                        ...
                                        print('Loss 1 contiguous: ', loss_1.is_contiguous())
